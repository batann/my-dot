...
fun! Start()
    ...
    let cmd = "find /home/batan ~/Documents ~/Downloads ~/Music ~/Coding ~/Archives ~/Pictures -maxdepth 1 -type f -cmin -360 -mmin -360 -amin -360 ! -name '.X*' ! -name '.xsession*' ! -name '.dmrc' ! -name '.less*' ! -name '.viminfo'"
    ...
    " Add a note
    call append('$', '')
    call append('$', "### Press <Enter> on a file to edit ###")

    " Make buffer read-only
    setlocal nomodifiable nomodified

    " Add mappings to open files with <Enter>
...
" Function to open a file when pressing <Enter> on a line
fun! OpenFile()
    let line = getline('.')
